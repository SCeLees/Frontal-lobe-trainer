...
export const gameRegistry = {
  ...
  'go-no-go': {
    ...
    rules: [
      '屏幕中央会出现字母 P 或 R（约70%为P，30%为R）',
      '看到 P → 立即按空格键或点击按钮',
      '看到 R → 不要做任何操作',
      'P 占多数会形成冲动倾向，训练你主动抑制误触',
    ],
  ...
  'flanker': {
    name: 'Flanker 任务',
    icon: '🏹',
    description: '只看中间箭头的方向，忽略两侧干扰箭头，快速按左右键作答。训练选择性注意与抑制控制。',
    rules: [
      '屏幕出现5个箭头，只有最中间的箭头是目标',
      '中间箭头朝左 → 按左方向键或点「← 左」',
      '中间箭头朝右 → 按右方向键或点「→ 右」',
      '两侧箭头有时与目标一致、有时相反，请忽略干扰',
      '每题有反应时限，超时算错；共20题',
    ],
    component: () => import('../games/Flanker.vue'),
    difficulty: {
      label: '反应时限',
      options: [
        { label: '慢速 (2.0s)', value: 2000 },
  ...
  'dccs': {
    name: 'DCCS 卡片分类',
    icon: '🎨',
    description: '颜色与形状独立随机组合成卡片，每 总轮数/4 轮循环切换颜色/形状分类规则，测试认知灵活性与规则切换能力。',
    rules: [
      '每局从6种颜色、6种形状中随机抽取2-4种，并随机组合成彩色形状卡片',
      '每 总轮数/4 轮切换一次规则：颜色 → 形状 → 颜色 → 形状 循环',
      '分类按钮固定显示「颜色+形状」，按当前规则匹配对应维度',
      '规则切换时会闪烁提示，请尽快适应新规则',
    ],
    ...
    difficulty: {
      label: '颜色/形状数量',
      options: [
        { label: '2 种（随机抽取）', value: 2 },
        { label: '3 种（随机抽取）', value: 3 },
        { label: '4 种（随机抽取）', value: 4 },
      ],
      default: 2,
    },
    extraOptions: {
  ...
  'trail-making': {
    name: '交替连线',
    icon: '🔗',
    description: '按 1→A→2→B→3→C 的数字字母交替顺序点击节点。这是经典连线测验（TMT-B），训练任务切换与认知灵活性。',
    rules: [
      '网格中随机散布数字1-N和字母A到第N个字母',
      '按 1 → A → 2 → B → 3 → C 的顺序依次点击',
      '点错会抖动并计一次错误，但可以继续',
      '全部节点点击完成后结束，用时越短、错误越少越好',
    ],
    component: () => import('../games/TrailMaking.vue'),
    result: {
      scoreLabel: '完成节点',
      totalLabel: '总节点',
      accuracyLabel: '最快用时',
      accuracyUnit: 'time',
      bestMetric: 'time',
      achievement: { type: 'clean', field: 'wrongClicks' },
    },
    difficulty: {
      label: '数字/字母对数',
      options: [
        { label: '5 组 (1-5·A-E)', value: 5 },
        { label: '7 组 (1-7·A-G)', value: 7 },
        { label: '9 组 (1-9·A-I)', value: 9 },
      ],
      default: 5,
    },
  },
  'task-switch': {
    name: '任务切换',
    icon: '🔀',
    description: '根据“奇偶/大小”提示判断数字，提示规则不断切换，考验任务切换与认知灵活性。',
    rules: [
      '屏幕同时显示规则提示和数字',
      '提示为「奇偶」时：奇数按左，偶数按右',
      '提示为「大小」时：小于5按左，大于5按右',
      '规则每轮随机切换，需要在不同任务间快速转换',
      '每题有反应时限，超时算错；共24题',
    ],
    component: () => import('../games/TaskSwitch.vue'),
    difficulty: {
      label: '反应时限',
      options: [
        { label: '慢速 (2.5s)', value: 2500 },
        { label: '正常 (2.0s)', value: 2000 },
        { label: '快速 (1.5s)', value: 1500 },
      ],
      default: 2000,
    },
  },
  'serial-addition': {
    name: '连续加法',
    icon: '🧮',
    description: '数字逐个出现，需要记住上一个数字，并计算“上一个数字 + 当前数字”。训练工作记忆与持续注意。',
    rules: [
      '先显示第一个数字，请记住它',
      '之后每出现一个数字，都要用上一个数字加上当前数字',
      '在输入框内输入相加结果并按回车或确认',
      '上一个数字不会再次显示，必须靠工作记忆保持',
      '答对得分，答错显示正确答案后继续',
    ],
    component: () => import('../games/SerialAddition.vue'),
    difficulty: {
      label: '题目数量',
      options: [
        { label: '12 题', value: 12 },
  ...
  'stop-signal': {
    ...
    rules: [
      ...
      '约30%的试次中，Go 出现后短暂延迟会突现「🚫 停止」',
      '停止信号出现前就响应、或出现后仍响应，都算误触',
      '停止信号出现后保持不响应，才算成功抑制',
      '停止信号出现越晚越难抑制',
    ],
    ...
    difficulty: {
      label: '停止信号延迟',
      options: [
        { label: '长延迟 (350ms·难)', value: 350 },
        { label: '中延迟 (250ms)', value: 250 },
        { label: '短延迟 (150ms·易)', value: 150 },
      ],
      default: 250,
    },
  },
  'memory-match': {
    ...
    result: {
      ...
      accuracyUnit: 'moves',
      bestMetric: 'moves',
      achievement: { type: 'moves', threshold: 1.5 },
    },
    difficulty: {
      label: '卡片对数',
      options: [
